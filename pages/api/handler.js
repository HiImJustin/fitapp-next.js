...
const handler = nc({
    ...
    if(session) {
    await loggingModel.checkTotalRequests(session.user.email)
        .then((results) => {
        console.log(results.length + " out of 1000 per 24hrs")
        //If the total length of requests in a 24hour period is greater than 1000 
        //Function will end - somehow logging in doesnt fail but everything else does
        if(results.length > 1000) { 
            console.log('stop spamming dickhead')
            res.status(401).json('too many request stop spamming me idiot')
            return;
        } else {
